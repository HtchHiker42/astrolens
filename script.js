// script.js — frontend logic for AstroLens
...
explainBtn.addEventListener('click', async () => {
  ...
  try {
    // IMPORTANT: use relative API route, Vercel will handle /api/ask
    ...
    // Image or NASA placeholder
    if (data.image_url) {
      imageContainer.innerHTML = `<img src="${data.image_url}" alt="related image" style="max-width:100%; border-radius:6px;">`;
    } else if (data.nasa_search) {
      imageContainer.innerHTML = `<a href="${data.nasa_search}" target="_blank">Search NASA images</a>`;
    } else {
      imageContainer.innerHTML = 'No image available';
    }

    // Visualization
    drawVisualization(data.visualization, concept);
...
function drawVisualization(viz = {}, concept = '') {
  ...
  if (viz.type === 'orbit' || concept.toLowerCase().includes('orbit')) {
    ...
    ctx.fillStyle = 'rgba(130,200,255,1)';
  ...
  else if (viz.type === 'lifecycle' || concept.toLowerCase().includes('star')) {
    const stages = ['Protostar', 'Main sequence', 'Red giant'];
